...
vec3 DirectionalLight(vec3 normal, vec3 direction, vec3 sint, vec3 viewDir, vec3 pos, vec3 env, vec3 envLight, vec3 diffuse)
{
  ...
  //float intensity =
	...
	vec2 ggx = ggxSpecular(normal, viewDir, direction, wCalc, 0.17);
...
void main()
{
  ...
  vec3 dl = DirectionalLight(normal, rVector, specularGloss.rgb, viewDir, position, env, envLight, diffuse.rgb);
